...
class GradNavDynamic:
    """GradNav algorithm with dynamic obstacle support.

    Extends GradNav to pass dynamic_objects_cfg to DynamicDroneEnv,
    enabling training with dynamic obstacles (sphere, box, cylinder).
    """
    def __init__(self, cfg):
        ...
        # Extract dynamic objects configuration
        dynamic_objects_cfg = cfg["params"].get("dynamic_objects", None)
        ...
        # Log dynamic objects status
        if hasattr(self.env, 'use_dynamic_objects') and self.env.use_dynamic_objects:
            print_info(f'[GradNavDynamic] Dynamic objects enabled')
        else:
            print_info(f'[GradNavDynamic] Dynamic objects not enabled (check config)')
    ...
    def save(self, filename = None):
        ...
        # Portable state_dict format - can be loaded across different repos
        portable_checkpoint = {
