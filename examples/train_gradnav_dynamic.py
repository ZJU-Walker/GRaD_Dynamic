...
# gradient-based policy optimization by actor critic method with dynamic obstacle support
import sys, os
...
import envs
from algorithms import gradnav_dynamic
import os
import sys
import yaml
import torch
...
def get_args(): # TODO: delve into the arguments
    custom_parameters = [
        ...
        {"name": "--cfg", "type": str, "default": "./cfg/gradnav/drone_dynamic_cylinder.yaml",
        ...
        {"name": "--logdir", "type": str, "default": "logs/tmp/gradnav_dynamic/"},
...
if __name__ == '__main__':
    ...
    if not args.no_time_stamp:
        map_name = cfg_train["params"]["config"].get("map_name", 'gate_mid')
        args.logdir = os.path.join(os.path.join(args.logdir, map_name),
                                   get_time_stamp())
    ...
    traj_optimizer = gradnav_dynamic.GradNavDynamic(cfg_train)
