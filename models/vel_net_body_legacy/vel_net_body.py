"""
Velocity Network (VELO_NET_BODY) for drone velocity estimation in BODY FRAME.

Auto-regressive GRU-based network that directly predicts velocity changes in body frame.
...
The network directly predicts velocity change from visual and IMU features:
    v_t = v_{t-1} + Network(obs)  (all in body frame)

Where:
- obs: Includes normalized IMU acceleration as an input feature (indicator)
- Network(obs): Directly predicts delta_v (velocity change in body frame)
...
Architecture:
    Input (76 dims) → LayerNorm → Projector MLP → GRU → Head MLP → Delta-V (3D, body frame)
    Final: vel_pred = prev_vel + delta_v (body frame)
"""
...
class VELO_NET_BODY(nn.Module):
    """
    Velocity estimation network with direct delta-v prediction in BODY FRAME.

    Uses GRU to process observation history and outputs velocity change (delta_v) in body frame.
    Implements direct prediction: vel = prev_vel + delta_v (all in body frame)
    ...
    def __init__(self,
                 ...
                 dt: float = 1.0 / 30.0,
                 device: str = 'cpu'):
        super(VELO_NET_BODY, self).__init__()
    ...
    @classmethod
    def load_checkpoint(cls, path: str, device: str = 'cpu') -> Tuple['VELO_NET_BODY', dict]:
        """
