...
class IMUAugmentation:
    ...
    def __init__(
        self,
        bias_range: float = 0.001,    # Uniform(-bias_range, bias_range) per axis
        scale_range: float = 0.001,   # Scale in [1-scale_range, 1+scale_range]
        noise_std: float = 0.001,     # White noise std
        dropout_prob: float = 0.001,  # Probability of zeroing accel
        enabled: bool = True,
    ):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict with:
                ...
                - velocities_gt: (L, 3) ground truth velocities in BODY frame
                - initial_prev_vel: (3,) GT prev_vel for first frame in BODY frame
                - velocities_world: (L, 3) ground truth velocities in WORLD frame
                - initial_prev_vel_world: (3,) GT prev_vel for first frame in WORLD frame
                - accel_gt: (L, 3) ground truth acceleration in BODY frame
                - accel_aug: (L, 3) augmented/corrupted acceleration in BODY frame
        """
        ...
        # Transform initial_prev_vel using the orientation at that timestep (start-1)
        ...
        # Compute ground truth acceleration from velocity derivative (world frame first)
        ...
        accel_world = (vel_curr - vel_prev) / self.dt  # (L, 3) in world frame
        ...
        return {
            ...
            # Body frame velocities (for training)
            'velocities_gt': velocities_gt,
            'initial_prev_vel': initial_prev_vel,
            # World frame velocities (for future usage)
            'velocities_world': velocities_world,
            'initial_prev_vel_world': initial_prev_vel_world,
...
def collate_sequences(batch: List[dict]) -> Dict[str, any]:
    ...
    return {
        ...
        # World frame velocities (for future usage)
        'velocities_world': [b['velocities_world'] for b in batch],
        'initial_prev_vels_world': [b['initial_prev_vel_world'] for b in batch],
        # Acceleration (body frame)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"  velocities_gt (body): {sample['velocities_gt'].shape}")
        print(f"  velocities_world: {sample['velocities_world'].shape}")
        print(f"  accel_gt (body): {sample['accel_gt'].shape}")
        print(f"  accel_aug (body): {sample['accel_aug'].shape}")

        # Show body vs world frame comparison
        print(f"\n  Body vs World frame (first frame):")
        print(f"    vel_world:  {sample['velocities_world'][0].tolist()}")
        print(f"    vel_body:   {sample['velocities_gt'][0].tolist()}")
