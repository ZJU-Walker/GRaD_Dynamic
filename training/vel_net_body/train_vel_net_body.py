...
def train_command(args):
    ...
    from training.vel_net_body.dataset import create_dataloaders
    from training.vel_net_body.trainer import VelNetTrainer, autoregressive_test
    ...
    # Create model (76 dims: no action, with IMU fusion)
    model = VELO_NET_BODY(
        num_obs=76,  # 6 + 3 + 32 + 32 + 3 (rot6d + prev_vel + rgb + depth + accel)
        stack_size=1,
    ...
    if sequences:
        ...
        ar_metrics = autoregressive_test(
            ...
            device=device,
            max_steps=9999,  # Test on full sequence
        )
...
def test_command(args):
    ...
    from training.vel_net_body.trainer import autoregressive_test
...
def eval_command(args):
    ...
    from training.vel_net_body.evaluator import fly_and_evaluate
...
def main():
    ...
    train_parser.add_argument('--checkpoint_dir', type=str, default='checkpoints/vel_net_body',
    ...
    train_parser.add_argument('--seq_length', type=int, default=64,
                              help='Sequence chunk length (0 = full sequences)')
    ...
    eval_parser.add_argument('--output_dir', type=str, default='output/vel_net_body_eval',
