...
def fly_and_evaluate(
    ...
    Args:
        model: Trained VELO_NET_BODY model
    ...
    print(f"\n{'='*60}")
    print(f"Body Frame Velocity Network Evaluation Flight")
    print(f"{'='*60}")
    ...
    for step in range(max_steps):
        ...
        vel_gt_world = state['velocity'][0].cpu().numpy()  # World frame
        quat_xyzw = state['orientation'][0].cpu().numpy()
        omega = state['angular_velocity'][0].cpu().numpy()

        # Transform velocity to body frame for comparison with model predictions
        ...
        thrust, omega_cmd, _ = controller.compute_from_quaternion(
            pos, vel_gt_world, quat_wxyz, omega,  # Controller uses world frame
        ...
        if step >= stabilize_steps:
            with torch.no_grad():
                ...
                # Compute IMU acceleration from GT velocity derivative (body frame)
        ...
        # Record frame for video with velocity overlay
        if step % 2 == 0:
            if rgb_np.max() <= 1.0:
            ...
            # Add velocity text overlay (after stabilization when we have predictions)
            if step >= stabilize_steps and len(vel_pred_list) > 0:
                ...
                vel_diff = vel_pred_curr - vel_gt_curr
                err_norm = np.linalg.norm(vel_diff)
                ...
                # GT velocity (green)
                gt_text = f"GT:   [{vel_gt_curr[0]:+.2f}, {vel_gt_curr[1]:+.2f}, {vel_gt_curr[2]:+.2f}]"
                cv2.putText(frame, gt_text, (x_pos, y_start), font, font_scale, (0, 255, 0), thickness)

                # Predicted velocity (cyan)
                pred_text = f"Pred: [{vel_pred_curr[0]:+.2f}, {vel_pred_curr[1]:+.2f}, {vel_pred_curr[2]:+.2f}]"
                cv2.putText(frame, pred_text, (x_pos, y_start + line_height), font, font_scale, (255, 255, 0), thickness)

                # Difference (red/yellow based on error)
                color = (0, 0, 255) if err_norm > 0.3 else (0, 165, 255) if err_norm > 0.15 else (0, 255, 255)
                diff_text = f"Diff: [{vel_diff[0]:+.2f}, {vel_diff[1]:+.2f}, {vel_diff[2]:+.2f}] |{err_norm:.3f}|"
                cv2.putText(frame, diff_text, (x_pos, y_start + 2*line_height), font, font_scale, color, thickness)
...
def save_velocity_plot(
    ...
    axes[0].set_title(f'Body Frame Velocity Prediction (Auto-regressive) | MAE: {metrics["mae"]:.4f} m/s | RMSE: {metrics["rmse"]:.4f} m/s')
